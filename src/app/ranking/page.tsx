...
function RankingContent() {
  ...
  useEffect(() => {
    ...
    const tabs: ("team1" | "team2" | "team3" | "team4")[] = [
      "team1",
      "team2",
      "team3",
      "team4",
  ...
  useEffect(() => {
    const fetchPlayerScores = async () => {
      try {
        ...
        // キャッシュを無効化
        const response = await fetch(
          `/api/playerscores?limit=1000&_t=${Date.now()}`,
          {
            cache: "no-store",
          }
        );
    ...
    if (rankings.length === 0) {
      return (
        <div className="text-center py-12">
          <p className="text-[#9aa3a6] text-lg tracking-wider">
            NO DATA AVAILABLE
          </p>
        </div>
      );
                    ...
                    <span
                      className={`w-10 h-10 rounded-lg grid place-items-center text-sm font-semibold tracking-wider uppercase transition-all duration-250 ${
                        index === 0
                          ...
                          : index === 1
                          ? "bg-gradient-to-b from-[rgba(35,40,55,0.85)] to-[rgba(15,18,25,0.95)] text-[#eef1ff] border-[rgba(190,210,255,0.45)]"
                          : index === 2
                          ? "bg-gradient-to-b from-[rgba(45,25,20,0.85)] to-[rgba(25,10,5,0.95)] text-[#ffe0c0] border-[rgba(255,180,120,0.4)]"
                      ...
                      style={{
                        ...
                        boxShadow:
                          index === 0
                            ? "inset 0 0 14px rgba(255, 220, 120, 0.25), 0 0 18px rgba(255, 200, 90, 0.4), 0 0 6px rgba(255, 255, 180, 0.3)"
                            : index === 1
                            ? "inset 0 0 14px rgba(190, 210, 255, 0.25), 0 0 18px rgba(150, 170, 255, 0.35), 0 0 6px rgba(220, 230, 255, 0.25)"
                            : index === 2
                            ? "inset 0 0 14px rgba(255, 150, 100, 0.25), 0 0 18px rgba(255, 130, 70, 0.4), 0 0 6px rgba(255, 200, 160, 0.25)"
                            : "",
                      }}
                    >
                      {index + 1}
